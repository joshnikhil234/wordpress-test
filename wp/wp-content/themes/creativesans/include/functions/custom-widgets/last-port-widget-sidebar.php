<?php
/**
 * Plugin Name: SAINTDO Last Portfolio (Sidebar)
 ...
 * @since 0.1
 */
add_action( 'widgets_init', 'last_portfolio_sidebar_widget' );
 ...
 * @since 0.1
 */
function last_portfolio_sidebar_widget() {
	register_widget( 'Last_Portfolio_Sidebar' );
}
 ...
 * @since 0.1
 */
class Last_Portfolio_Sidebar extends WP_Widget {

	/**
	 * Widget setup.
	 */
	function Last_Portfolio_Sidebar() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'lastport-sidebar-widget', 'description' => __('A widget that show last items from your portfolio.', 'lastport-sidebar-widget') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'lastport-sidebar-widget' );

		/* Create the widget. */
		$this->WP_Widget( 'lastport-sidebar-widget', __('Last Portfolio (Sidebar) SAINTDO', 'lastport-sidebar'), $widget_ops, $control_ops );
	}
					...
					<?php if(have_posts()) : ?>
						<?php while(have_posts()) : the_post();?>				
									...
									<div class="last-port-wrapper-widget <?php if($i%2 == 0) echo " last"; $i++;?>"> 
												...
												?>
												<div class="port-postthumb h70 w112">
													<a href="<?php the_permalink();?>" title="<?php the_title();?>" class="tipsy-hover" >	
														<?php //Check wheter thumbnail exists ?>
														<?php if($image_id  == null):?>
															<img src="<?php echo get_template_directory_uri(); ?>/scripts/timthumb.php?src=<?php echo get_template_directory_uri(); ?>/img/default.jpg&amp;h=70&amp;w=112&amp;zc=1" alt="<?php the_title(); ?>"  class="thumb hover"/>
														<?php else: ?>
															<img src="<?php echo get_template_directory_uri(); ?>/scripts/timthumb.php?src=<?php echo $image_url[0];?>&amp;h=70&amp;w=112&amp;zc=1" alt="<?php the_title(); ?>"  class="thumb hover"/>
														<?php endif ?>
													</a>
												</div>								
